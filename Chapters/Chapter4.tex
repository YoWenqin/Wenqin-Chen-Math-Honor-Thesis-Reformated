...
All the same postulates still apply for density matrices. Here's \textbf{Postulate 2} in terms of density matrices:
\begin{quote}
    Say the evolution of a closed quantum system is described by a unitary transformation U. Then the evolution of the density operator is described by the equation
    \begin{equation}
        \rho=\sum_i p_i \ket{\psi_i}\bra{\psi_i}	\underrightarrow{U}\sum_i p_i U \ket{\psi_i}\bra{\psi_i}U^\dagger=U \rho U^\dagger
    \end{equation}
\end{quote}

Here's how we will describe measurements (\textbf{Postulate 3}) in terms of density matrices:
\begin{quote}
    Suppose we perform a measurement described by measurement operators $M_m$ on an ensemble of pure states $\{p_i, \ket{\psi_i}\}$. If the initial state prepared was $\ket{\psi_i}$, then the probability of getting result m is
    \begin{equation}
...
A quantum system whose state $\ket{\psi}$ is known exactly is said to be in a \textit{pure state}. In this case the density matrix representation is simply $\rho=\ket{\psi}\bra{\psi}$. Otherwise, we say $\rho$ is in a \textit{mixed state}. It is a \textit{mixture} of the different pure states in the ensemble for $\rho$. We will mostly only discuss pure states.
