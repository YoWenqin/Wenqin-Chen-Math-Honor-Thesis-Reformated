...
Bob's decipher process using the same one-time pad can be illustrated by \textbf{Table \ref{table: one-time pad decoding}}, following the decryption scheme also defined in \textbf{Definition \ref{def: classical one-time pad scheme}}:
\begin{table}[h] 
\centering
\begin{tabular}{|c|c|c|c|c|c|}
\hline
