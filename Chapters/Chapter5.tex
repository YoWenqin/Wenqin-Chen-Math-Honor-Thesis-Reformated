...
\textcolor{green}{Now let's introduce the quantum one-time pad, the quantum analogue of the classical one-time pad encryption scheme. You can refer to   \cite{mosca2000private} for mode in-depth discussion. Here we just describe the encryption scheme. Recall that to transmit n classical bits privately via the classical one-time pad, n bits of shared private key is necessary and sufficient. It turns out to transmit n qubits privately, 2n bits of shared private key are necessary and sufficient.}
...
\textcolor{blue}{Include example of this computation.  How does this compare to the classical one-time pad?  Also, does the No Cloning Theorem get used for something later?} \textcolor{green}{it's mentioned in BB84}
...
\textcolor{blue}{Inside the marked area must be modified.  Maybe the stuff about non-locality should be in the introduction, but I can't figure out where.  The thing is that I think it's better to describe the game really carefully pretty early}
...
A classical strategy is simply given by functions $f_A(x)=a$ and $f_B(y)=b$. By simply checking all possible choices in the following table, we see the maximum winning probability is $\frac{3}{4}$.
...
\textcolor{green}{Abiding the same set of rules as in the classical strategy that no communication is allowed after the game starts, Alice and Bob are now able to improve their winning probability by deploying a quantum strategy! There are two differences from the classical strategy. First, Alice and Bob share an entangled 2 qudit state.\footnote{You can can just imagine Alice and Bob gather in a physics labs before the game start, fire a beam through a special kind of crystal and obtain a pair of entangled photons. Then Alice and Bob each take one photon and walk away from each other to start the game.} Second, Alice and Bob make measurements with respect to different orthonormal bases depending on the question they each receive.}
