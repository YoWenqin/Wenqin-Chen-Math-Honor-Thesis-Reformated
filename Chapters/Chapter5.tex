...
Generally, we can classify encryption schemes as either \emph{private key cryptography} or \emph{public key cryptography}. In private key cryptography, the same secret key is used for Alice's encryption and Bob's decryption, whereas in public key cryptography, one key is used to encrypt and is made available to the public, a different secret key is used to decrypt.

Currently, pubic key cryptosystems such as the \emph{RSA cryptosystem} are the most widely used cryptographic protocol now.  Public key cryptosystems don't rely on Alice and Bob sharing a secret key in advance. Instead, Bob generates a pair of keys, and makes one of them available to the public. Alice can then use that public key to encrypt her message, which Bob can easily decipher with his other key that remains private. It is extremely difficult for Eve to guess the Bob's other private key with access only to the public key. Such security relies on classical computer's difficulty in solving the problem of factoring when large primes are involved. It's worth noting that {\emph{Shor's fast algorithm}} on a quantum computer can easily break RSA.
