...
\end{proof}
\end{example}
In {\bf{Chapter}} \ref{Chapter6-classification of entanglement} we will classify entanglement completely. For now, let's just point out a few important properties of entanglement. 

Introduced in \cite{Toner2006}, '''monogamy ''' is used in cryptography protocols such as E91 which we will discuss in \textbf{Section \ref{section: e91}}. Here is what it means. If two qubits A and B are maximally entangled with each other, they cannot be correlated at all with a third qubit C. In general, there is a trade-off between the amount of entanglement between qubits A and B and the same qubit A and qubit C.

\textcolor{blue}{maybe add more properties of entanglement here. entanglement implies correlation.}
