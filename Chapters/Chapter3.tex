...
For our purposes, the Hilbert space is $C^n$. In fact, often $n=2$, so the state vector is a qubit (see \textbf{Definition \ref{def qubit}} and more generally Chapter \ref{Chapter2-preliminaries}).  Thus, in many of our examples, a state vector is a superposition of $\ket{0}$ and $\ket{1}$.
...
This time, say Alice selects from the states $\ket{\psi_1}=\ket{0}$ and  $\ket{\psi_2}=\frac{\ket{0}+\ket{1}}{\sqrt{2}}=\ket{+}$. Suppose Bob attempts to distinguish $\ket{\psi_1}, \ket{\psi_2}$ with measurement operators $M_1=\ket{0}\bra{0}, M_2=\ket{1}\bra{1}$ as before.
