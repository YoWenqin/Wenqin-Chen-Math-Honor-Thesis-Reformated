...
For our purposes, the Hilbert space is $C^n$. In fact, often $n=2$, so the state vector is a qubit (see \textbf{Definition \ref{def qubit}} and more generally \textbf{Chapter \ref{Chapter2-preliminaries}}).  Thus, in many of our examples, a state vector is a superposition of $\ket{0}$ and $\ket{1}$.
...
\footnote{
$\ket{+}$ and $\ket{-}$ can both be viewed as being "halfway" between $\ket{0}$ and $\ket{1}$} of $\ket{0}$ and $\ket{1}$ (see \cite{Nielsen}).
{\color{green}
In fact, $\ket{+}, \ket{-}$ form a commonly used orthonormal basis in $\mathbb{C}^2$ called \textit{Hadamard basis}. They are defined as follows:
...
\end{align}
This computation shows that Bob can detect the outcome $i$ with probability $1$.  Once again the crucial point is that the set of vectors from which Alice selects is known to Bob and is {\emph{orthonormal}}. Because of this, he can measure with respect to the corresponding collection of measurement operators. 
\end{proof}
...
\begin{definition}[Measuring in a Basis]\label{def: measurement in a basis state vector}\footnote{Measuring in a basis actually belongs to a special subclass of quantum measurement called \textit{projective measurement}. We will define projective measurements rigorously in the \textbf{Appendix \ref{AppendixB}}, but for our purposes it is sufficient just to introduce measuring with respect to an orthonormal basis.} 
To measure in a basis $\{\ket{u_m}\}$, where $\ket{u_m}$ form an orthnormal basis, means to perform a quantum measurement with a collection $\{M_m\}$ of measurement operators such that $M_m=\ket{u_m}\bra{u_m}$.

If the quantum system is in state $\ket{\psi}$ immediately prior to measurement, then the probability of that outcome m occurs is given by
\begin{equation}\label{eqn: basis measurement probability}
    p(m)=|\braket{\psi|b_i}|^2.
\end{equation}
When this is the case, the state of the system after the measurement is $\ket{u_m}$
\end{definition}

We can check that this definition is consistent with \textbf{Postulate 3}.
Notice that $$M_m^2=\ket{u_m}\bra{u_m}\ket{u_m}\bra{u_m}=\ket{u_m}\braket{u_m|u_m}\bra{u_m}=\ket{u_m}\bra{u_m}=M_m$$ and $$M_m^\dagger=(\ket{u_m}\bra{u_m})^\dagger=\ket{u_m}\bra{u_m}=M_m$$.

Therefore, $$\sum_m M_m^\dagger M_m=\sum_m M_m^2=\sum_m M_m = \sum_m \ket{u_m}\bra{u_m}=\mathbb{I}.$$
We proved the correctness of the last step back in \textbf{Section \ref{section:complex vector space}}. 
So measuring in a basis does satisfy the completeness equation required for quantum measurement as defined in \textbf{Postulate 3}. 
You can easily check \eqref{eqn: general measurement probability} becomes \eqref{eqn: basis measurement probability} and that \eqref{eqn: general measurement post state} becomes $\ket{u_m}$ up to a global phase. The calculation is very similar to the one shown in \textbf{Example \ref{measurement-standard basis}}.

While \textbf{Example \ref{measurement-standard basis}} shows a measurement with respect to the standard basis, here is an additional exmple of measuring on the Hadamard basis.
