...
\end{equation}
This says that {\emph{any}} hermitian matrix can be written in the above form.
\begin{example}
\begin{equation}
...
Thus, a projector is a projection in the sense of linear algebra, so its eigenvalues are $0$ and $1$.  Also, if $\ket{w} \in W$, then $P\ket{w}=\ket{w}$.
